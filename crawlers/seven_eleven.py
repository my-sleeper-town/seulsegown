...
def crawl_seven_eleven():
    ...
    서울에 있는 세븐일레븐 점포를 찾아 반환합니다.
        Returns: [{'jumpo_name': 점포이름, 'street_address': 주소, 'latitude': 위도, 'longitude': 경도}]
    '''
    jumpos = []
    ...
    store_lay_sido = Select(driver.find_element(By.ID, 'storeLaySido'))
    store_lay_sido.select_by_value('서울')
    ...
    WebDriverWait(driver, 20).until(EC.presence_of_all_elements_located(
        (By.XPATH, '//select[@id="storeLayGu"]/option'))
    )
    store_lay_gu = Select(driver.find_element(By.ID, 'storeLayGu'))
    store_button1 = driver.find_element(By.ID, 'storeButton1')

    options = [option.get_attribute('value') for option in store_lay_gu.options][1:]
    ...
    for option in options:
        ...
        store_lay_gu.select_by_value(option)
        store_button1.click()

        WebDriverWait(driver, 40).until(EC.presence_of_element_located((By.ID, 'layer')))
        WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(
            (By.XPATH, '//div[@class="list_stroe"]/ul/li/a'))
        )
        ...
        for jumpo in jumpo_links:
            WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(
                (By.XPATH, '//div[@class="list_stroe"]/ul/li/a/span'))
            )
